...
int main(int argc, char *argv[])
{
	...
	// ################################ Ex5 Debug Vector ######################################
	if(myrank == 0) cout << "##################################################### Ex5 Debug Vector" << endl;
	
	int MyVectorSize = 4;
	vector<double> vec(MyVectorSize); 
	...
	// ########################## Ex6 Dot Product #############################
	if(myrank == 0) cout << "##################################################### Ex6 Dot Product" << endl ;
	double innerProduct = 0.0;
	size_t const N = 10;								  // VectorSize DotProduct
	...
	if(myrank == 0) {
		cout << "Process: " << myrank << " -- Print Distributed Inner Product: " << innerProduct << endl;
		cout << "Global size of vectors: " << N << endl;	
		assert(static_cast<int>(innerProduct) == N);
	...
	if(myrank == 0) cout << "##################################################### Ex7 MinMaxExchange" << endl ;
	...
	if (myrank == 0) localVec = {4.0,2.0,40.0,1000.0, 1.0};
	if (myrank == 1) localVec = {5.0,-59.0,-1000.0,-1000.0, 23.0, 11.0, 1.0};
	if (myrank == 2) localVec = {1000.0,-1000.0,-78.0};
	if (myrank == 3) localVec = {-50.0, 1000.0,21.0,0.0, 2.0};
	...
	//MPI_Alltoall(x.data(), 5, MPI_DOUBLE, exchanged.data(), 5, MPI_DOUBLE, icomm);
	MPI_Alltoall(MPI_IN_PLACE, 5, MPI_DOUBLE, x.data(), 5, MPI_DOUBLE, icomm);

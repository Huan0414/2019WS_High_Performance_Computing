...
class Mesh
{
    public:
       ...
       /**
		 * Identify the index the node closest to coordinates (@p xin, @p yin).
         * The distance has to be smaller than @p eps .
         *
		 * @param[in] xin  coodinate
         * @param[in] yin  pair
         * @param[in] accuracy
         * @return index or -1 if distace to large.
		*/  
       int FindClosestNode(double const xin, double const yin, double const eps=1e-2) const;
       
       virtual void DebugValueAtCoords(std::vector<double> const & w, 
               double const xin, double const yin, double const eps=1e-2) const;

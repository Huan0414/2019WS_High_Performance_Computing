...
void Mesh::SetValues(vector<double> &v, const function<double(double, double)> &func) const
{
...
void Mesh::SetBoundaryValues(vector<double> &v, const function<double(double, double)> &func) const
{
    auto const idx = Index_BoundaryNodes();
    for (size_t ik = 0; ik < idx.size(); ++ik)
    {
        const int k = idx[ik];
        v[k] = func( _xc[2 * k], _xc[2 * k + 1] );
    }
...
void Mesh::SetDirchletValues(vector<double> &v, const function<double(double, double)> &func) const
{
    auto const idx = Index_DirichletNodes();
    for (size_t ik = 0; ik < idx.size(); ++ik)
    {
        const int k = idx[ik];
        v[k] = func( _xc[2 * k], _xc[2 * k + 1] );
...
int Mesh::FindClosestNode(double const xin, double const yin, double const eps) const
{
    double const eps2 = eps*eps;
    auto const &  xy = GetCoords();
    int const nnodes = Nnodes();
    assert(xy.size()==2*static_cast<size_t>(nnodes));
    
    double vmin{1e30};
    int    imin{-1};
    for (int k=0; k<nnodes; ++k)
    {
        double const dist2 = pow(xy[2*k]-xin,2) + pow(xy[2*k+1]-yin,2);
        if (dist2<vmin)
        {
            vmin = dist2;
            imin = k;
...
void Mesh::DebugValueAtCoords(vector<double> const & w, double const xin, double const yin, 
            double const eps) const
{
    int const imin = FindClosestNode(xin,yin,eps);
    auto const &xy = GetCoords();
    if (imin>=0)
    {
        cout << "Node[" << imin << "] (" << xy.at(2*imin) << ", "<< xy.at(2*imin+1) << ")   value: " << w.at(imin) << "\n";
...
/**
 * Visualize @p v together with its mesh information via matlab or ocatve.
  *
...
std::vector<int> Mesh::Index_DirichletNodes() const
{
    ...
    // 2020-01-08
    //                 copy only the Dirichlet boundary nodes, not all boundary nodes.
    vector<int> idx(_bedges.size());
    size_t cnt=0;
    for (size_t kb = 0; kb < _bedges.size(); kb+=2 )
    {
        if (_sdedges.at(kb) <0 || _sdedges.at(kb+1) <0) // one neighboring subdomain is negativ
        {
            idx[cnt] = _bedges[kb];
            ++cnt;
            idx[cnt] = _bedges[kb+1];
            ++cnt;
        }
    }
    idx.resize(cnt);
...
void Mesh::DeriveEdgeFromVertexBased_fast_2()
{
    ...
    //}
    // 2020-01-08
    // GH ToDo: Selection of Dirichlet edges/nodes is still wrong
    //          Problem: _bedges is used externally instead of the local correct bdir.
    for (size_t kb = 0; kb < _bedges.size(); kb+=2 )
    {
        bool const booldir = _sdedges.at(kb) <0 || _sdedges.at(kb+1) <0; // one neighboring subdomain is negativ
        bdir.at(_bedges[kb])   = booldir;
        bdir.at(_bedges[kb+1]) = booldir;
    }
...
// 2020-01-08
    int const DUMMY{-9876};
    _sdedges.resize(nbedges * 2,DUMMY);
    if (!ifs.eof())
    {
        cout << nbedges << endl;           // change to  while-loop
        for (int k = 0; k < nbedges * 2; ++k)
        {
            ifs >> _sdedges.at(k);
            _sdedges[k] -= OFFSET;            // Matlab to C indexing
        }
        //assert(ifs.eof());
        assert(count(_sdedges.cbegin(),_sdedges.cend(),DUMMY)==0);
    }
    else
    {
...
RefinedMesh::RefinedMesh(Mesh const &cmesh, vector<bool> const &ibref)
...
vector<int> Mesh_2d_3_square::Index_DirichletNodes() const
{
    int const dx = 1,
              dy = _nx + 1,
              bl  = 0,
...
{
    cerr << "Warning: Funktion not implemented.  " << __FILE__ << "." << __LINE__ << endl;
    return Index_BoundaryNodes();
}



/**
  * Stores the values of vector @p u of subdomain @p myid into a file.

...
void JacobiSolveParallel(ParMesh const &cmesh, CRS_Matrix const &SK, vector<double> const &fv, vector<double> &uv, MPI_Comm const &icomm)
{
	const double omega   = 1.0;
    const int    maxiter = 1000;
    const double tol  = 1e-6,                // tolerance
    ...
    int nrows = SK.Nrows();                  // number of rows == number of columns
    assert( nrows == static_cast<int>(fv.size()) && fv.size() == uv.size() );
    ...
    SK.GetDiag(dd);                          //  dd := diag(K)
    cmesh.VecAccu(dd);						 // Accumulated dd
    ////DebugVector(dd);{int ijk; cin >> ijk;}

    //  Initial sweep
    SK.Defect(r, fv, uv);                      //  r := f - K*u	
	
    vddiv(w, r, dd);                         //  w := D^{-1}*r
    cmesh.VecAccu(w);						 // Accumulated w
    
    const double sigma0 = dscapr(w, r);      // s0 := <w,r>
    double AccuSigma0;
	MPI_Allreduce(&sigma0,&AccuSigma0,1,MPI_DOUBLE, MPI_SUM, icomm);
    // Iteration sweeps
    int iter  = 0;
    double sigma = 0;
    double AccuSigma = AccuSigma0;
    while ( AccuSigma > tol2 * AccuSigma0 && maxiter > iter)  // relative error
        //while ( sigma > tol2 && maxiter > iter)         // absolute error
    {
        ++iter;
        vdaxpy(uv, uv, omega, w );             //  u := u + om*w
        SK.Defect(r, fv, uv);                  //  r := f - K*u
        
        vddiv(w, r, dd);                     //  w := D^{-1}*r
        cmesh.VecAccu(w);					 // Accumulated w
        sigma = dscapr(w, r);                // s0 := <w,r>
        MPI_Allreduce(&sigma,&AccuSigma,1,MPI_DOUBLE, MPI_SUM, icomm); // Reduction of sigmal inside iterations
//      	cout << "Iteration " << iter << " : " << sqrt(sigma/sigma0) << endl;
    }
    cout << "aver. Jacobi rate :  " << exp(log(sqrt(sigma / sigma0)) / iter) << "  (" << iter << " iter)" << endl;
    cout << "final error: " << sqrt(sigma / sigma0) << " (rel)   " << sqrt(sigma) << " (abs)\n";
